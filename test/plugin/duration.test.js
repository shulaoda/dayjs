...
import dayjs, { duration, relativeTime } from '../../dist';
...
describe('Creating', () => {
  ...
  it('should handle floating point rounding errors', () => {
    ...
    expect(dayjs.duration(7647826.525774224).toISOString()).toBe(
      'PT2H7M27.827S',
    ); // was PT2H7M27.826999999999998S
  });
});
...
test('Subtract duration', () => {
  ...
  const p = dayjs.duration('P1Y1M1DT1H1M1S');
  expect(b.subtract(p).format('YYYY-MM-DD HH:mm:ss')).toBe(
    '2022-02-01 01:01:01',
  );
});
