...
import dayjs, { pluralGetSet } from '../../dist';
...
it('Years', () => {
  expect(dayjs().get('years')).toBe(moment().get('year'));
  expect(dayjs().years()).toBe(moment().year());
  expect(dayjs().years(0).valueOf()).toBe(moment().year(0).valueOf());
  expect(dayjs().years(2000).valueOf()).toBe(moment().year(2000).valueOf());
});

it('Months', () => {
  expect(dayjs().get('months')).toBe(moment().get('month'));
  expect(dayjs().months()).toBe(moment().month());
  expect(dayjs().months(0).valueOf()).toBe(moment().month(0).valueOf());
  expect(dayjs().months(1).valueOf()).toBe(moment().month(1).valueOf());
});
...
it('Dates', () => {
  expect(dayjs().get('dates')).toBe(moment().get('date'));
  expect(dayjs().dates()).toBe(moment().date());
  expect(dayjs().dates(0).valueOf()).toBe(moment().date(0).valueOf());
  expect(dayjs().dates(1).valueOf()).toBe(moment().date(1).valueOf());
});
...
it('Set Dates', () => {
  expect(dayjs().date(30).valueOf()).toBe(moment().date(30).valueOf());
...
it('Set Months', () => {
  expect(dayjs().months(11).valueOf()).toBe(moment().month(11).valueOf());
