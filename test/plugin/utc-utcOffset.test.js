...
it('Set utcOffset -> Get utcOffset', () => {
  ...
  expect(dayjs().utcOffset(-60).format()).toBe(
    moment().utcOffset(-60).format(),
  );
  expect(dayjs().utcOffset(-8).format()).toBe(moment().utcOffset(-8).format());
});
...
test('utc startOf', () => {
  ...
  expect(
    moment(d).utc().utcOffset(480).endOf('day')
      .valueOf(),
  )
    .toBe(
      dayjs(d).utc().utcOffset(480).endOf('day')
        .valueOf(),
    );
