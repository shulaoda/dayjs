...
import dayjs, { negativeYear, REGEX_PARSE, utc } from '../../dist';
...
describe('negativeYear', () => {
  ...
  it('Add and subtract with negative years', () => {
    ...
    expect(dayjs('-2006').add(1, 'y').format('YYYY')).toBe(
      dayjs('-2005').format('YYYY'),
    );
    expect(dayjs('-2006').subtract(1, 'y').format('YYYY')).toBe(
      dayjs('-2007').format('YYYY'),
    );
  });
