...
import dayjs, { customParseFormat, utc } from '../../dist';
...
describe('UTC Offset', () => {
  ...
  it('get utc offset with a negative valid string value, format: HH:mm', () => {
    const time = '2021-02-28 19:40:10';
    const hoursOffset = -8;
    const daysJS = dayjs(time).utc().utcOffset(
      `-0${Math.abs(hoursOffset)}:00`,
      true,
    );
    const momentJS = moment(time).utc(true).utcOffset(
      `-0${Math.abs(hoursOffset)}:00`,
      true,
    );
