...
import dayjs, { customParseFormat, timezone, utc } from '../../dist';
...
describe('Convert', () => {
  it('convert to target time', () => {
    ...
    const MlosAngeles = moment('2014-06-01T12:00:00Z').tz(
      'America/Los_Angeles',
    );
...
describe('startOf and endOf', () => {
  ...
  it('preserves locality when tz is called', () => {
    ...
    expect(tzWithoutLocality.startOf('week').format('YYYY-MM-DD')).toEqual(
      '2023-02-12',
    );
    expect(tzWithLocality.startOf('week').format('YYYY-MM-DD')).toEqual(
      '2023-02-15',
    );
  });
});
...
describe('UTC timezone', () => {
  it('TZ with UTC with Locale', () => {
    const test1 = dayjs('2000-01-01T09:00:00+09:00').tz('Asia/Seoul').locale(
      'en',
    );
    expect(test1.hour()).toBe(9);
    const test2 = dayjs('2000-01-01T09:00:00+09:00').tz('Asia/Hong_Kong')
      .locale('en');
